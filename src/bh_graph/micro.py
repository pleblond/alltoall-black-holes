...
from bh_graph.horizon import horizon_area, horizon_radius

FOUR_PI = 4.0 * np.pi


def critical_k(r_point: float = 1.0, lp: float = 1.0) -> float:
    """Max legs embeddable through a point region: k_crit = 4 pi r_point^2 / lp^2."""
    return float(FOUR_PI * r_point**2 / lp**2)


def is_pointlike(k, r_point: float = 1.0, lp: float = 1.0) -> np.ndarray | bool:
...
def quantized_area(k, lp: float = 1.0, area_gap: float = 1.0, r_point: float = 1.0):
